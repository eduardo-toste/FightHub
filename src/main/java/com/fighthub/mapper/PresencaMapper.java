package com.fighthub.mapper;

import com.fighthub.dto.presenca.PresencaResponse;
import com.fighthub.model.Aluno;
import com.fighthub.model.Aula;
import com.fighthub.model.Presenca;
import org.springframework.data.domain.Page;

public class PresencaMapper {

    public static PresencaResponse toDTO(Presenca presenca, Aluno aluno, Aula aula) {
        return new PresencaResponse(
                presenca.getId(),
                presenca.isPresente(),
                presenca.getInscricao().getId(),
                aluno.getId(),
                aluno.getUsuario().getNome(),
                aula.getId(),
                aula.getTitulo(),
                presenca.getDataRegistro()
        );
    }

    public static Page<PresencaResponse> toPageDTO(Page<Presenca> presencas) {
        return presencas.map(presenca -> {
            var inscricao = presenca != null ? presenca.getInscricao() : null;
            Aluno aluno = inscricao != null ? inscricao.getAluno() : null;
            Aula aula = inscricao != null ? inscricao.getAula() : null;
            return toDTO(presenca, aluno, aula);
        });
    }

}
